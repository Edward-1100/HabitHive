...
router.post('/:id/edit', isLoggedIn, upload.single('avatar'), async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      if (existingUser && existingUser.profileImage && existingUser.profileImage.startsWith('/uploads/avatars/')) {
        const oldFs = path.join(process.cwd(), 'public', existingUser.profileImage);
        try {
